...
public interface MockControllerDocs {
  ...
  @ApiChangeLogs({
    @ApiChangeLog(
      ...
      issueUrl = "https://github.com/Team-TicketMate/ticketmate-server/issues/624"
    )
  })
  @Operation(
    summary = "테스트 알림 발송 (fcm)",
    description = """
      이 API는 `인증(JWT)` 및 `fcm 토큰`이 필요합니다.
      
      ### 요청 파라미터 (MockNotificationRequest)
      - `title` (String): 발송할 알림 제목 [선택]
      - `body` (String): 발송할 알림 본문 [선택]
      
      ### 사용 방법 & 유의 사항
      - 로그인 및 JWT 토큰이 필요합니다.
      - FCM 토큰 사전 등록이 필요합니다. (`/api/fcm` POST)
      - title, body는 입력하지 않으면 기본 값으로 발송됩니다.
      """
  )
  void sendTestNotification(
    CustomOAuth2User customOAuth2User,
    MockNotificationRequest request
  );
}
